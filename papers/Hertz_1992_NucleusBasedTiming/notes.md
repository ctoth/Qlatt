...
- [Rhythm in Read British English: Interdialect Variability](../Ferragne_Pellegrino_2004_RhythmReadBritishEnglish/notes.md) - Strong. Both papers address multi-dialect duration modeling from opposite directions: Hertz & Huffman build a rule-based synthesis timing model that explicitly separates language-universal, dialect-universal, and dialect-specific duration components (with duration distributed via a trading relationship within the acoustic nucleus); Ferragne & Pellegrino instead show empirically, via automatically-extracted PVI/duration measures on real British-dialect recordings, that within one language it is specifically vowel-duration variability (not consonant-duration variability) that carries dialect-distinguishing rhythmic signal - exactly the kind of empirical grounding a "dialect-specific" duration component in Hertz & Huffman's architecture should be built from.
