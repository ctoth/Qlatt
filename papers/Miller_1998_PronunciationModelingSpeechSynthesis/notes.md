...
- [[Fitt_Isard_1999_SynthesisRegionalEnglishKeywordLexicon]] — Fitt & Isard use the same lexical/postlexical split Miller's dissertation is built on, but for accent instead of speaker-specific allophony: one accent-neutral keyword lexicon plus hand-authored per-accent postlexical rules generates Scottish, Irish, RP, etc. surface forms, whereas Miller trains a neural network to learn one speaker's postlexical variation from data. The two papers bracket the design space for what the postlexical layer should be (rule-authored vs. learned) and are complementary evidence that "lexical form → postlexical rules → surface form" is the right architectural cut for a declarative TTS frontend. Miller's bibliography also cites two earlier Fitt papers (1995, 1997) on regional and unfamiliar-name pronunciation.
- [Robust LTS rules with the Combilex speech technology lexicon](../Richmond_Clark_Fitt_2009_RobustLTSCombilex/notes.md) — Richmond, Clark & Fitt evaluate the other half of the pipeline Miller's dissertation addresses: where Miller learns postlexical (lexical-to-surface) realization for known words, Combilex's evaluation is about improving letter-to-sound (LTS) prediction for words absent from the lexicon, using an explicit expert phone-grapheme alignment carried through the lexicon's base-to-surface accent derivation. Both papers treat lexicon/rule consistency as directly measurable in downstream accuracy — Miller via 87.8%/98% exact/acceptable postlexical prediction, Combilex via 86.50% vs 84.49% LTS words-correct from alignment quality alone.
- [Representing the Environments for Phonological Processes in an Accent-Independent Lexicon for Synthesis of English](../Fitt_Isard_1998_RepresentingEnvironmentsPhonologicalProcesses/notes.md) — Fitt & Isard's 1998 discussion paper (same year as Miller's dissertation) independently arrives at the same lexical-vs-postlexical division question Miller's thesis is built around, but resolves it by linguistic argument (a phonemic-vs-allophonic heuristic, with practical grounds breaking ties) rather than by training a model on data; Miller's entropy-based finding that /t/ and schwa are hardest to predict is an empirical echo of exactly the hard boundary cases (vowel reduction, /t/-allophony) this paper identifies as difficult to allocate between lexicon and rule.
- [The Treatment of Vowels Preceding 'r' in a Keyword Lexicon of English](../Fitt_1999_TreatmentVowelsPrecedingR/notes.md) — Fitt's ICPhS 99 paper supplies an explicit two-criterion rule (Principle I: lexicalize if phonemic in any accent; Principle II: derive by rule if predictable from environment, overriding Principle I) for exactly the kind of lexicon/postlexical-rule allocation decision Miller's neural network learns empirically per-phenomenon; comparing the two is a useful test of whether Miller's learned allocation (which phones are hardest to predict, i.e. /t/ and schwa) lines up with what a linguist's explicit criteria would also flag as rule-resistant.
