...
import { z } from "zod";
import type { BaconConnection, BaconGraph, BaconNode } from "./klatt-runtime";
...
export const formantBanksSchema = z
  .record(nonEmptyString, formantBankSchema)
  .superRefine((banks, context) => {
 ...
 */
export function expandFormantBanks(graph: BaconGraph, semantics: SemanticsDocument): void {
  ...
  for (const [, bank] of Object.entries(banks)) {
    const sortedFormants = [...bank.formants].sort((a, b) => a.index - b.index);
    ...
    for (const f of sortedFormants) {
      ...
      const N = f.index;
      graph.connections.push([f.parallelSource, `parallelF${N}`] as BaconConnection);
      graph.connections.push([`parallelF${N}`, `parallelF${N}Gain`] as BaconConnection);
      graph.connections.push([`parallelF${N}Gain`, bank.parallel.output] as BaconConnection);
    }
