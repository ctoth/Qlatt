...
async function initAudio() {
  ...
  try {
    ...
    debugLog(
      `AudioContext created. State: ${audioContext.state}, Sample Rate: ${audioContext.sampleRate}`
    );
...
function setupConfigListeners() {
  ...
  const handleSwChange = () => {
    const swValue = parseInt(
      document.querySelector('input[name="swMode"]:checked').value,
      10
    );
...
function speakText() {
  ...
  setTimeout(() => {
    ...
    try {
      ...
      debugLog(
        `Using TTS params: BaseF0=${baseF0}, TransitionMs=${transitionMs}`
      );
      ...
      console.log(
        "[Main UI] Full Klatt track stored in 'window.lastKlattTrack'. Inspect it in the console after execution."
      );
      // *** END ADDED ***
      ...
      setTimeout(() => {
        ...
        if (
          ...
          debugLog(
            "Auto-stop condition not met (synth not running or status changed), not stopping."
          );
      ...
      if (trackDisplay)
        trackDisplay.textContent = `Error generating track: ${error.message}`;
...
function updateSynthStatusDisplay() {
  if (!klattSynth || !klattSynth.isRunning) {
    synthStateDisplay.textContent = `Synth State: ${
      klattSynth?.isRunning
        ? "Running (No Track)"
        : klattSynth?.isInitialized
        ? "Idle"
        : "Uninitialized"
    }`;
  ...
  synthStateDisplay.textContent = `Synth State: Speaking
Time: ${state.currentTime.toFixed(3)}s (Track: ${state.elapsedTrackTime.toFixed(
    3
  )}s)
...
function stopStatusUpdates() {
  if (animationFrameId !== null) {
    ...
    if (synthStateDisplay)
      synthStateDisplay.textContent = `Synth State: ${
        klattSynth?.isInitialized ? "Idle" : "Uninitialized"
      }`;
  }
}
...
speakTextButton.disabled = true;
if (synthStateDisplay)
  synthStateDisplay.textContent = "Synth State: Uninitialized"; // Initial display text
