...
- 2026-02-14: expanded `typecheck:audio` across the PLSTEP runtime chain worklets (edge detect, envelope decay, branch switch) and removed one stale field.
- Evidence: `src/worklets/edge-detector-processor.ts`, `src/worklets/decay-envelope-processor.ts`, and `src/worklets/signal-switch-processor.ts` now have strict option/class-field/process/message typing, explicit WASM instantiate narrowing, and null-safe WasmBuffer view handling.
- Evidence: dead code cleanup in this pass removed the unused edge-detector field `_lastTriggered`; `tsconfig.audio.json` now includes all three PLSTEP chain worklets.
- Evidence: verification remains green via `npm run typecheck:audio`, `npm run typecheck:golden`, `npm run typecheck:core`, `npm run test:golden`, and `npm test`.
- 2026-02-14: expanded `typecheck:audio` to additional sample-by-sample excitation/filter worklets (`impulsive`, `square`, `triangular`, `pitch-sync-mod`, `tilt-filter`).
- Evidence: `src/worklets/impulsive-source-processor.ts`, `src/worklets/square-source-processor.ts`, `src/worklets/triangular-source-processor.ts`, `src/worklets/pitch-sync-mod-processor.ts`, and `src/worklets/tilt-filter-processor.ts` now have strict option/class-field/process/message typing and explicit WASM instantiate narrowing for `Instance | WebAssemblyInstantiatedSource`.
- Evidence: `tsconfig.audio.json` now includes all five processors, and verification remains green via `npm run typecheck:audio`, `npm run typecheck:golden`, `npm run typecheck:core`, `npm run test:golden`, and `npm test`.
- 2026-02-14: expanded `typecheck:audio` to remaining WASM-heavy auxiliary worklets (`aerodynamic-model`, `fujisaki-resonator`, `oversampled-glottal-source`) and removed dead telemetry fields in aerodynamic processor.
- Evidence: `src/worklets/aerodynamic-model-processor.ts`, `src/worklets/fujisaki-resonator-processor.ts`, and `src/worklets/oversampled-glottal-source-processor.ts` now have strict option/class-field/process/message typing, explicit WASM instantiate narrowing, and null-safe WasmBuffer view handling.
- Evidence: dead code cleanup in this pass removed unused aerodynamic processor fields (`debug`, `reportInterval`, `_reportCountdown`) that had no reporting path.
- Evidence: `tsconfig.audio.json` now includes these processors, and verification remains green via `npm run typecheck:audio`, `npm run typecheck:golden`, `npm run typecheck:core`, `npm run test:golden`, and `npm test`.
- 2026-02-14: `typecheck:core` coverage expanded to include the declarative runtime engine implementation.
- Evidence: `src/declarative-frontend/engine.ts` now has strict-typing fixes for previously implicit-`any` / unknown spread sites (typed rule/timing helpers, object narrowing for splice template evaluation, and typed runtime/trace setup in `runRuleEngine`).
- Evidence: `tsconfig.core.json` now includes `src/declarative-frontend/engine.ts`, and `npm run typecheck:core` remains green after the expansion.
- 2026-02-14: `typecheck:core` coverage expanded again to include declarative phoneme inventory materialization.
- Evidence: `src/declarative-frontend/inventory.ts` now has explicit typing for `fillDefaultParams` and `materializePhonemeTarget`, including safe key narrowing for base-parameter overrides and typed dynamic payload augmentation fields.
- Evidence: `tsconfig.core.json` now includes `src/declarative-frontend/inventory.ts`, and `npm run typecheck:core` remains green with both engine + inventory in gate scope.
- 2026-02-14: expanded `typecheck:core` to include the production frontend entry (`src/tts-frontend.ts`) and resolved dictionary typing overflow.
- Evidence: `src/tts-frontend.ts` now has explicit function parameter/return types, typed frontend token aliases, typed CMU/phoneme map access, and deterministic casts at declarative frontend boundaries so strict mode no longer reports implicit-`any`/union narrowing failures.
- Evidence: `src/cmu-dictionary.ts` now exports `CMU_DICT` with explicit `Record<string, string>` annotation, eliminating TS7056 inferred-type serialization overflow under strict checks.
- Evidence: `tsconfig.core.json` now includes `src/tts-frontend.ts`, and `npm run typecheck:core` stays green after the expansion.
- 2026-02-14: expanded `typecheck:core` to include runtime diagnostics + semantic runtime dependencies (`src/diagnostics.ts`, `src/klatt-runtime.ts`) with strict-safe CEL builtin adapters.
- Evidence: `src/diagnostics.ts` now has explicit `DiagnosticEntry`/listener/options interfaces, typed methods, and strict-safe listener/entry collections (no implicit `any` paths remain).
- Evidence: `src/klatt-runtime.ts` now uses explicit `AudioWorkletNode` narrowing helpers for readiness/telemetry paths and numeric-argument wrapper registrations for CEL builtins (`dbToLinear`, `dbToLinearKlsyn`, `min`, `max`, `pow`) compatible with `ParamValue` function signatures.
...
- 2026-02-14: expanded `typecheck:core` to include remaining semantics barrel export (`src/semantics/index.ts`) and the legacy runtime synth implementation (`src/klatt-synth.ts`).
- Evidence: `src/klatt-synth.ts` now has explicit class field declarations (`ctx/options/nodes/params/telemetry/worklet state`), strict method signatures for scheduling/telemetry/WASM helpers, typed `import.meta.env` access, and strict-safe worklet-ready promise handling (`Promise<void>` with typed message events).
- Evidence: `src/klatt-synth.ts` helper typing pass also resolves strict nullability hazards in telemetry and parallel-formant application paths (typed optional handler capture, optional `freq` argument default, guarded worklet timeout logging).
- Evidence: `tsconfig.core.json` now includes both `src/semantics/index.ts` and `src/klatt-synth.ts`, and verification remains green via `npm run typecheck:core`, `npm run typecheck:audio`, `npm run typecheck:golden`, `npm run test:golden`, and `npm test`.
- 2026-02-14: added a third layered strict gate for non-golden utility scripts (`typecheck:scripts`) and typed the first included scripts.
