...
function dump(phrase: string) {
  ...
  const v = t.filter((f) => ((f.params?.F0 as number) ?? 0) > 0);
  const n = v.length;
  const pts: string[] = [];
  for (let i = 0; i < 10; i++) {
    const f = v[Math.floor((i * n) / 10)];
    pts.push(`${((f.params?.F0 as number)).toFixed(0)}`);
  }
  const f0s = v.map((f) => f.params?.F0 as number);
  return `${phrase}\n  decile F0: [${pts.join(" ")}]  min=${Math.min(...f0s).toFixed(0)} max=${Math.max(...f0s).toFixed(0)}`;
}
for (const p of ["you are home.", "one two three four five.", "the quick brown fox jumps over the lazy dog."]) {
  console.log(dump(p));
}
