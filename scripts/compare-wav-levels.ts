 ...
 */
import { readWav, estimateLag, alignByLag, rmsError, maxAbsError } from "./oracle/wav.ts";
...
console.log(`A: ${pathA}`);
console.log(`   samples=${a.samples.length} peak=${sa.peak.toFixed(5)} (${db(sa.peak)}) rms=${sa.rms.toFixed(5)} (${db(sa.rms)})`);
console.log(`B: ${pathB}`);
console.log(`   samples=${b.samples.length} peak=${sb.peak.toFixed(5)} (${db(sb.peak)}) rms=${sb.rms.toFixed(5)} (${db(sb.rms)})`);
...
console.log(`aligned rmsError=${rmsError(reference, candidate).toFixed(5)} maxAbsError=${maxAbsError(reference, candidate).toFixed(5)}`);
console.log(`peak ratio B/A=${(sb.peak / sa.peak).toFixed(3)} rms ratio B/A=${(sb.rms / sa.rms).toFixed(3)}`);
