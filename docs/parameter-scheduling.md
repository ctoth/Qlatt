...
The optional `plstep` section in semantics configures interpreter burst telemetry.
`triggers` lists frame parameters tracked independently; `initialValue` starts each
track and `missingValue` applies when a frame omits a trigger parameter. `comparison`
is `gte` or `gt`, comparing the signed delta against `constants.plstepThreshold`.
The policy requires `citations`, which accompany emitted events. Without a policy,
the interpreter emits no PLSTEP events. An empty trigger list disables detection.

Bundled policies preserve AF-only detection at a rise of at least 49 dB, with zero
initial/missing values (Klatt 1980 PARCOE.FOR and the existing host off convention).
Telemetry reads `plstepAmplitude` and `plstepAmplitudeDb` from realized semantics;
it does not compute a second amplitude formula. Burst audio is still generated by
the experiment's graph, so changing this telemetry policy does not change audio.
