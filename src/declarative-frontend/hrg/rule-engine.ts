...
function projectPolicyValues(value: unknown): unknown {
  ...
  if (Object.hasOwn(value, "value")) return projectPolicyValues(value.value);
  ...
  private readonly boundFunctions = new Proxy<Record<string, (...args: unknown[]) => unknown>>(
    {},
    {
      get: (_target, property) =>
        typeof property === "string" ? this.activeFunctions?.[property] : undefined,
    },
  );
...
function buildEvaluationContext(options: EvaluationContextOptions): EvaluationContext {
  ...
  const findWithinWord = (
    ...
    const directions: readonly (-1 | 1)[] =
      directionName === "behind" ? [-1] : directionName === "both" ? [1, -1] : [1];
...
function normalizeCelValue(value: unknown, isItemView: (value: unknown) => boolean): unknown {
  ...
  if (typeof value === "bigint") {
    ...
    if (!Number.isSafeInteger(number)) {
      throw new Error(
        `E_HRG_CEL_INTEGER_RANGE: ${value.toString()} is outside the safe integer range`,
      );
    }
    return number;
  }
  if (value instanceof Map) {
    return Object.fromEntries(
      [...value.entries()].map(([key, nested]) => [
        String(key),
        normalizeCelValue(nested, isItemView),
      ]),
    );
  }
...
function evaluateDispatch(
  ...
  for (const branch of value.dispatch) {
    ...
    if (Object.hasOwn(branch, "default")) {
...
function updateNestedValue(
  ...
  const update = (source: FeatureValue | undefined, index: number): unknown => {
...
function applyEffects(
  ...
  for (const effect of effects) {
    ...
    const scalarConfigs =
      isPlainObject(relationSpec) && isPlainObject(relationSpec.scalars)
    ...
    if (resolution === "klatt" && scalarConfig) {
      ...
      if (!Number.isFinite(floor)) {
        floor =
          typeof scalarConfig.min === "number" && Number.isFinite(scalarConfig.min)
            ? scalarConfig.min
            : 0;
      }
    }
...
function makePhraseResetPredicate(
  ...
  const resetBreakIndex = typeof block.reset_break_index === "number" ? block.reset_break_index : 4;
...
function attachContourContexts(
  ...
  if (!isPlainObject(rule.contour) || rule.contour.domain !== "phrase" || matches.length === 0)
    return;
...
function applySplice(
  ...
  for (let index = 0; index < splice.insert.length; index += 1) {
    ...
    const templateFields =
      segment && isPlainObject(segment.fields)
        ? {
            ...segment.fields,
            ...(Object.hasOwn(segment, "phoneme") ? { phoneme: segment.phoneme } : {}),
          }
        : template;
...
function applyPointActions(
  ...
  for (let index = 0; index < actions.length; index += 1) {
    ...
    if (typeof relationName !== "string" || relationName.length === 0) {
      if (f0Layer)
        throw new Error("E_HRG_F0_CONTROL_RELATION: f0_layer insert requires a relation");
      continue;
    }
    ...
    if (itemTypes.length !== 1) {
      throw new Error(
        `E_HRG_POINT_ITEM_TYPE: relation '${relationName}' must declare exactly one Item type`,
      );
    }
    ...
    if (f0Layer) {
      ...
      if (Object.hasOwn(spec, "duration_frames")) {
...
function patternMatches(
  ...
  if (
    ...
    typeof pattern.relation !== "string" ||
...
function finalizePhase(
  ...
  if (phase.compute_times) {
    ...
    for (const item of items) {
      const anchor = utterance.intervalAnchor(item);
      if (!anchor)
        throw new Error(`E_TIME_ANCHOR_REQUIRED: Item '${item.id}' has no interval anchor`);
      transaction.dependOn(anchor.decisionId);
      ...
      if (typeof duration !== "number" || !Number.isFinite(duration) || duration < 0) {
        throw new Error(
          `E_TIME_DURATION_REQUIRED: Item '${item.id}' has no finite non-negative duration`,
        );
      }
