...
- [Emotional Speech Synthesis: A Review](../Schroder_2001_EmotionalSpeechSynthesisReview/notes.md) - (Strong) direct continuation of the rule-based, perception-optimized approach the review surveys via Burkhardt's PhD thesis and workshop paper (refs [8],[9], source of the review's joy and fear rule sets): this paper develops that same method into an explicit voice-quality parameter framework for formant synthesis.
- [The Generation of Affect in Synthesized Speech](../Cahn_1990_AffectEditor/notes.md) - (Strong) Cahn is the historical precedent for exactly this project: a rule-based, perceptually-validated mapping from emotion/voice-quality descriptors to synthesizer control parameters. Cahn maps qualitative affect labels to DECtalk3 settings without physical units; this paper supplies the missing physically-grounded Klatt formulas (OQ, spectral tilt, formant bandwidths, amplitudes) for the phonation-type half of the same emotion-to-parameter problem.
