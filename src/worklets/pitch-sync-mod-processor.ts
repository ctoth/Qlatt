...
class PitchSyncModProcessor extends AudioWorkletProcessor {
  ...
  f1Buffer: WasmBuffer | null;
  b1Buffer: WasmBuffer | null;
  dF1Buffer: WasmBuffer | null;
  dB1Buffer: WasmBuffer | null;
  ready: boolean;
  debug: boolean;
  nodeId: string;
  ...
  process(
    ...
    if (hasNonstandardRates) {
      for (let i = 0; i < blockSize; i += 1) {
        outputChannel[i] = this.wasm.pitch_sync_resonator_process(
          this.state,
          inputChannel[i] || 0,
          f0.length > 1 ? (f0[i] ?? firstF0) : firstF0,
          oq.length > 1 ? (oq[i] ?? firstOq) : firstOq,
          f1.length > 1 ? (f1[i] ?? firstF1) : firstF1,
          b1.length > 1 ? (b1[i] ?? firstB1) : firstB1,
          dF1.length > 1 ? (dF1[i] ?? firstDF1) : firstDF1,
          dB1.length > 1 ? (dB1[i] ?? firstDB1) : firstDB1,
          skew.length > 1 ? (skew[i] ?? firstSkew) : firstSkew,
          source.length > 1 ? (source[i] ?? firstSource) : firstSource,
        );
      }
    ...
    this.wasm.pitch_sync_resonator_process_block(
      this.state,
