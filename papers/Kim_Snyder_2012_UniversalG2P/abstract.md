...
This paper tackles automatic grapheme-to-phoneme (G2P) prediction for unknown languages by learning from 107 languages with Latin alphabets. The key insight is that phonotactic patterns (sound sequencing rules) are cross-linguistically universal enough to generalize: textual patterns from an unknown language can reveal its underlying phoneme inventory using a probabilistic graphical model trained on multiple languages. The work is directly relevant to speech synthesis—accurate G2P mappings are essential for TTS systems, especially for languages lacking pronunciation dictionaries.
