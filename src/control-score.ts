...
type TrajectoryControlField = {
  op: "set";
  value: number;
};

type TrajectoryControlWindow = {
  start_ms: number;
  end_ms: number;
...
function buildTrajectorySegments(
  ...
  for (let index = 0; index < points.length; index += 1) {
    ...
    const segmentValue = index === 0
      ? point.value
      : (previousPointValue + point.value) / 2;
...
function trajectoryFieldsEqual(
  left: Record<string, TrajectoryControlField>,
  right: Record<string, TrajectoryControlField>,
): boolean {
...
function roundTrajectoryBoundary(value: number): number {
  return Math.round(value * 1000) / 1000;
}

/**
 * Lower field trajectories into the same frontend-neutral timed-control window
 * shape consumed by buildDeclarativeControlScore().
 */
export function buildTrajectoryControlWindows(
  ...
  for (let index = 0; index + 1 < orderedBoundaries.length; index += 1) {
    ...
    if (previous && previous.end_ms === startMs && trajectoryFieldsEqual(previous.fields, fields)) {
