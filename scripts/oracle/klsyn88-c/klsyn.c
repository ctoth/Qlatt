...
/* Minimal 16-bit mono RIFF/WAV writer. The original writwave.c (edit 000005) is
 * absent from this source tree -- the Python/Cython path replaced it with scipy.
 * This restores standalone WAV output WITHOUT altering synthesis. */
static void write_wav(const char *fname, int16_t *samples, int nsamp, int samrate) {
    FILE *wf = fopen(fname, "wb");
    if (wf == NULL) { printf("\tERROR: cannot open '%s' for writing\n", fname); return; }
    uint32_t data_bytes = (uint32_t)nsamp * 2u;
    uint32_t sr = (uint32_t)samrate;
    uint32_t byte_rate = sr * 2u;
    uint32_t riff_size = 36u + data_bytes;
    uint32_t fmt_size = 16u;
    uint16_t audio_fmt = 1u, channels = 1u, block_align = 2u, bits = 16u;
    fwrite("RIFF", 1, 4, wf);
    ...
    fwrite(samples, 2, (size_t)nsamp, wf);
    fclose(wf);
    printf("\tWaveform saved in '%s' (%d samples @ %d Hz)\n", fname, nsamp, samrate);
}
...
//        printf("\n\tSynthesis requested");
        ...
        write_wav(wname, iwave, nsamtot, SAMRAT);
        if (batch) exit(0);
