...
class VoicingSourceProcessor extends AudioWorkletProcessor {
  ...
  constructor(options) {
    ...
    console.log(
      `[VoicingSource] Initialized (F0 Scaling Active, Ref F0=${this.referenceF0}). Sample Rate: ${sampleRate}`
    );
  }
  ...
  process(inputs, outputs, parameters) {
    ...
    if (!output || !output[0]) {
      // No output channel available
      console.warn("No output channel available.");
      return true;
    }
    ...
    for (let i = 0; i < blockLength; ++i) {
      ...
      const baseAmpDb =
        baseAmpDbValues.length > 1 ? baseAmpDbValues[i] : baseAmpDbValues[0];
      ...
      const f0Factor =
        f0ForScaling > 0 && this.referenceF0 > 0
          ? f0ForScaling / this.referenceF0
          : 1.0;
      const currentAmp = linearBaseAmp * f0Factor;

      const isActiveCurrentSample =
        f0 >= MIN_F0_HZ && baseAmpDb > MIN_AMP_DB && sampleRate > 0;
