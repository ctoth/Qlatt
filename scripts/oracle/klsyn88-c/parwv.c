...
/* FIDELITY FIX (Qlatt klsyn88 oracle): the original gen_noise() uses
 * (rand()>>17)-8192, which assumes a 31-bit RAND_MAX (~2^31) as on the VAX/Sun
 * platforms Klatt's klsyn ran on. This oracle is built with MinGW gcc whose
 * RAND_MAX is 32767 (15-bit), so rand()>>17 == 0 for every draw and the noise
 * collapses to a constant DC (-8192) -> no aspiration/frication/breathiness.
 * Replace host rand() with the SAME 31-bit ANSI-style LCG that Qlatt's
 * oversampled-glottal-source crate uses (rng_state = rng_state*1103515245+12345
 * & 0x7fffffff, advance-then-return), so the oracle's noise is (a) actually
 * present and (b) consistent with the Qlatt port. Seed handling mirrors the
 * crate's set_seed: state = seed (used directly, as the original srand(ranseed)). */
static unsigned int klsyn_rng_state = 1u;
static int klsyn_rand31(void) {
    klsyn_rng_state = (klsyn_rng_state * 1103515245u + 12345u) & 0x7fffffffu;
    return (int)klsyn_rng_state;
}
...
/*        Read Speaker Definition from Host */
            ...
            klsyn_rng_state = (unsigned int)ranseed;  /* Init 31-bit LCG (see top) */
        }
...
void gen_noise(void) {
	...
	nrand = (klsyn_rand31()>>17) - 8192;   /* 31-bit LCG, not host rand() (see top) */
