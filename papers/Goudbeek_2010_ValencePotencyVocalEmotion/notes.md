...
- [Acoustic Correlates of Emotion Dimensions in View of Speech Synthesis](../Schroder_2001_AcousticCorrelatesEmotionDimensions/notes.md) — Strong. Schröder et al. map speech acoustics onto the same activation/evaluation(-power) dimensional space Goudbeek uses, over a decade earlier and for synthesis rather than perception; both find that arousal/activation is carried by prosodic parameters (pitch, tempo, intensity) while valence/evaluation needs additional spectral cues, converging from production and perception sides of the same dimensional-emotion framework.
