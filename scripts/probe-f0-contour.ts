...
console.log(
  `--- F0 contour (frame: time_s -> Hz) [${full ? "full" : `last ${frames.length - start}`}] ---`,
);
...
console.log(
  `--- tail summary (last ${window} VOICED frames; voiced_end_frame=${voicedEnd - 1}) ---`,
);
console.log(
  `pre_window_F0=${preWindowF0.toFixed(2)} end_F0=${endF0.toFixed(2)} delta=${(endF0 - preWindowF0).toFixed(2)}`,
);
console.log(`rising_steps=${risingSteps} falling_steps=${fallingSteps}`);
console.log(
  `tail_direction=${endF0 > preWindowF0 + 1e-6 ? "RISE" : endF0 < preWindowF0 - 1e-6 ? "FALL" : "FLAT"}`,
);
