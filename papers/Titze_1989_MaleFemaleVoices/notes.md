...
- [[Lucero_2009_MaleFemaleVocalFoldSimulation]] — Strong. Kob et al. use a multiple-mass model (VOX) to simulate the *mechanical* consequences of the male-female anatomical differences that Titze characterizes here: where Titze derives acoustic output differences from scale factors, Kob shows that female vocal fold geometry produces spatially localized contact forces (hourglass pattern) vs. the distributed forces in male/linear geometries, connecting anatomy to vocal fold pathology (nodule prevalence).
