...
- [[Grabe_Kochanski_Coleman_2005_IntonationNativeAccentVarieties]] — **Strong.** Direct empirical evidence for Ladd's gradience critique: full per-pattern frequency tables for IViE dialect data (Cambridge/Newcastle/Belfast) show even the most frequent categorical AM pattern per (dialect, utterance-type) cell rarely exceeds 50-80% of tokens, with many cells (polar questions especially) split across 7-10 distinct patterns. Also directly invokes Ladd (1996)'s repeated-prenuclear-accent-collapsing convention in its own methodology.
